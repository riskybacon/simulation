...
namespace GL
{
   ...
   Program::Program(const std::string& vShaderFile, const std::vector<std::string>& varyings, GLenum buffermode)
   :  _vertexShader   (NULL)
   ...
   ,  _geometryShader (NULL)
   {
      ...
      char* cVaryings[varyings.size()];
      
      size_t i = 0;
      for(auto vPtr : varyings)
      {
         cVaryings[i] = new char[vPtr.length()];
         strncpy(cVaryings[i], vPtr.c_str(), vPtr.length());
         ++i;
      }

      glTransformFeedbackVaryings(_handle, varyings.size(), (const char **) cVaryings, buffermode);
      ...
      mapVaryingNamesToIndices();
      
      for(i = 0; i < varyings.size(); ++i)
      {
         delete cVaryings[i];
      }
   }
...
#define DEBUG
   ...
   void Program::mapUniformNamesToIndices(void)
   {
      ...
      {
         std::cout << "uniform: (name, index): (" << itr->first << "," << itr->second << ")" << std::endl;
      }
#endif
   }
   ...
   void Program::mapAttributeNamesToIndices(void)
   {
      ...
      for(GLuint i = 0; i < total; ++i)
      {
         ...
         name[name_len] = 0;
         _attrib[std::string(name)] = glGetAttribLocation(_handle, name);
      }
#ifdef DEBUG_ATTRIBUTES
      std::map<std::string, GLuint>::const_iterator itr;
      for(itr = _attrib.begin(); itr != _attrib.end(); ++itr)
      {
         std::cout << "attrib: (name, index): (" << itr->first << "," << itr->second << ")" << std::endl;
      }
#endif
   }
    ...
    * Build a mapping of uniform names to uniform indices
    */
   void Program::mapVaryingNamesToIndices()
   {
      
      int total = -1;
      glGetProgramiv(_handle, GL_TRANSFORM_FEEDBACK_VARYINGS, &total);
      for(int i=0; i<total; ++i)  {
         int name_len=-1, num=-1;
         GLenum type = GL_ZERO;
         char name[100];
         glGetTransformFeedbackVarying(_handle, GLuint(i), sizeof(name) - 1, &name_len, &num, &type, name);
         name[name_len] = 0;
         _varying[std::string(name)] = i;
      }

#ifdef DEBUG_VARYINGS
      for(auto varying : _varying)
      {
         std::cout << "varying: (name, index): (" << varying.first << "," << varying.second << ")" << std::endl;
