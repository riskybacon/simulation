...
#define ASSERT_ATTRIBUTE_EXISTS(_name)
   ...
   public:
      ...
      void enableVertexAttribArray(const std::string& name)
      {
         ASSERT_ATTRIBUTE_EXISTS(name);
#ifdef _DEBUG
         assert(_attrib[name] < GL_MAX_VERTEX_ATTRIBS);
#endif
         ...
         if(errnum > 0)
         {
            switch(errnum)
            {
               case GL_INVALID_OPERATION:
                  std::cerr << "GL_INVALID_OPERATION after glEnableVertexAttribArray. This may mean that this was called between glBegin() / glEnd() or that a vertex array object was not bound" << std::endl;
                  break;
               case GL_INVALID_VALUE:
                  std::cerr << "GL_INVALID_VALUE after glEnableVertexAttribArray. This means that the attribute index was >= GL_MAX_VERTEX_ATTRIBS (" << GL_MAX_VERTEX_ATTRIBS << ")" << std::endl;
                  break;
               default:
                  std::cerr << "Unknown error after glEnableVertexAttribArray. This OpenGL implementation returned an undocumented error code after a call to glEnableAttribeArray" << std::endl;
            }
         }
#endif
#endif
...
#if 0
      void setUniform(const std::string& name, const GLuint i0)
      {
         glUniform1ui(_uniform[name], i0);
      }
      ...
      void setUniformMatrix3(const std::string& name, GLsizei count, GLboolean transpose,
                             const GLfloat* value)
      {
#ifdef ROBUST_UNIFORM_LOCATIONS
         glUniformMatrix3fv(getUniformLocation(name), count, transpose, value);
#else
         glUniformMatrix3fv(_uniform[name], count, transpose, value);
#endif
      }
   ...
   {
   public:
      VertexAttribute(const int handle, const std::string& name)
      :  _handle  (handle)
      ,  _name    (name)
      {
#ifdef _DEBUG
         assert(handle < GL_MAX_VERTEX_ATTRIBS);
#endif
      }
      ...
      {
         glVertexAttribPointer(_handle, size, type, normalized, stride, pointer);
      }
      
   private:
      GLuint _handle;    //< OpenGL handle for a vertex attribute
      std::string _name; //< The name of this vertex attribute as specified in a shader
   };
}
#endif
